                ...
                event = es.next() => {
                    match event {
                        ...
                        Some(Err(e)) => {
                            ...
                            // Some providers (e.g. MiniMax) close the SSE stream without
                            // sending a [DONE] terminator. Treat "Stream ended" as a
                            // normal end so accumulated tool-call buffers are finalized.
                            if err_str.contains("Stream ended") || err_str.contains("stream ended") {
                                debug!("SSE stream closed without [DONE] — treating as normal end");
                                break;
                            }
